...
import { Box, Dialog, DialogActions, DialogContent, DialogContentText, DialogTitle, Grid, TextField, Tooltip, Typography } from '@mui/material';
...
const Themes = () => {
    ...
    const fetchData = async () => {
        try {
            ...
            // console.log(getThemeurl.data.data);
    ...
    const copyClick = () => {
        navigator.clipboard.writeText(`http://localhost:3000/${data?.slug}`)
            .then(() => {
                setCopyText('copied');
        ...
        <Box className={styles.bg}>
            <DashboardLayout>
                <Box>
                    ...
                    <ToastContainer />
                    <Grid container spacing={3} justifyContent={'center'} sx={{ margin: 0, gap: '20px' }}>
                        {themes.map((theme, index) => (
                            <Box key={index} onClick={() => { getThemeId(theme.id); handleClickOpen(); }}
                                sx={{ bgcolor: '#2c2c2c', p: 1, borderRadius: '10px', cursor: 'pointer' }}
                                style={{ border: selectedTheme === theme.id ? '4px solid green' : '4px solid transparent' }}
                            >
                                <Grid item xs={6} sx={{ color: '#fff' }}>
                                    <Image src={`${process.env.API_BASE_URL}/images/${theme.theme}`} alt="theme" width={500} height={400} style={{ borderRadius: '10px' }} />
                                    <Typography sx={{ fontSize: '19px', textAlign: 'center', textTransform: 'capitalize', fontWeight: '600', width: '500px' }}>{theme.name}</Typography>
                                </Grid>
                            </Box>
                        ))}
                    </Grid>
