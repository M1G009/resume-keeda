...
import { Skills, SkillsUpdate, getSkills, removeSkills } from '../services/Skills';
import style from "./forms.module.css"
import { ToastContainer, toast } from 'react-toastify';

const validationSchema = yup.object({
    name: yup.string().required('Name is required'),
    rate: yup.number().required('Level is required').min(1, 'Rate must be at least 1')
                ...
                <Box>
                    <form onSubmit={formik.handleSubmit} className='formborder'>
                        <Grid container spacing={2} justifyContent="center" alignItems="center">
                            <Grid item xs={6}>
                                <Box>
                                    <label htmlFor="name" className='text'>Skill Name</label>
                                    <Box>
                                        <input
                            ...
                            <Grid item xs={6}>
                                <Box>
                                    <label htmlFor="rate" className='text'>Level</label>
                                    <Box>
                                        <input
                                            className={style.input}
                                            id="rate"
                                    ...
                                    {formik.errors.rate && formik.touched.rate && (
                                        <div className={style.error}>{formik.errors.rate}</div>
                                    )}
                                </Box>
                            </Grid>
                    ...
                    <Table>
                        <TableHead>
                            <TableRow>
                                ...
                                <TableCell sx={{ fontWeight: 600, fontSize: '17px', textTransform: 'capitalize', textAlign: 'center' }}>rate</TableCell>
